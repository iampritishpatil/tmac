...
def get_fourier_freq(
    ...
    freqs = 2.0 * math.pi / float(t_max) * w_vec  # radians per *sample*
    return freqs
...
class PoolingMode(str, Enum):
    fixed = "fixed"         # taus provided (scalar or per-ROI) in *seconds*
...
class KernelName(str, Enum):
    rbf = "rbf"
    ou_ct = "ou_ct"     # continuous-time OU (AR(1) in continuous time)
    ar1_dt = "ar1_dt"   # discrete-time AR(1)


class TMACGPUOptions(BaseModel):
    # Core (all taus are in *seconds*)
    ...
    tau_a: Optional[Union[float, list[float]]] = Field(
        default=None, description="Activity time constant(s) in seconds (used when pooling='fixed')."
    )
    tau_m: Optional[Union[float, list[float]]] = Field(
        default=None, description="Motion time constant(s) in seconds (used when pooling='fixed')."
    )
    dt: float = Field(default=1.0, gt=0, description="Sampling interval in seconds.")
    kernel_a: KernelName = Field(default=KernelName.rbf, description="Kernel for activity.")
    kernel_m: KernelName = Field(default=KernelName.rbf, description="Kernel for motion.")

    # Hierarchical prior (log-normal via Gaussian on log τ in seconds)
    hier_mu_a: float = Field(
        default=0.0, description="Mean of log(tau_a [s]) for hierarchical pooling."
    )
    hier_sigma_a: float = Field(
        default=0.5, gt=0, description="Std of log(tau_a [s]) for hierarchical pooling."
    )
    hier_mu_m: float = Field(
        default=0.0, description="Mean of log(tau_m [s]) for hierarchical pooling."
    )
    hier_sigma_m: float = Field(
        default=0.5, gt=0, description="Std of log(tau_m [s]) for hierarchical pooling."
    )
    ...
    init_log_tau_a: float = Field(
        default=0.0, description="Initial log(tau_a [s]) when learned."
    )
    init_log_tau_m: float = Field(
        default=0.0, description="Initial log(tau_m [s]) when learned."
    )
...
# ---------------------------
# PSD helpers for kernels
# ---------------------------

def _psd_rbf(freqs_rs: torch.Tensor, tau: torch.Tensor, var: torch.Tensor) -> torch.Tensor:
    """
    RBF kernel spectral density (continuous-time form):
    S(ω) = σ^2 * τ * sqrt(2π) * exp(-0.5 * (ω τ)^2)
    freqs_rs: [F,1] radians per second
    tau, var: [1,N]
    """
    wt = freqs_rs * tau
    return var * (tau * math.sqrt(2.0 * math.pi)) * torch.exp(-0.5 * (wt ** 2))


def _psd_ou_ct(freqs_rs: torch.Tensor, tau: torch.Tensor, var: torch.Tensor) -> torch.Tensor:
    """
    OU (continuous-time AR(1)) spectral density:
    S(ω) = σ^2 * (2 τ) / (1 + (ω τ)^2)
    """
    wt = freqs_rs * tau
    return var * (2.0 * tau) / (1.0 + wt * wt + 1e-30)


def _psd_ar1_dt(freqs_rs: torch.Tensor, tau: torch.Tensor, var: torch.Tensor, dt: torch.Tensor) -> torch.Tensor:
    """
    Discrete-time AR(1) driven by white noise, with time-constant τ in seconds
    and sampling interval dt in seconds.
    φ = exp(-dt/τ)
    S(ω) = var / (1 + φ^2 - 2 φ cos(ω dt))
    Here freqs_rs is radians/second; we multiply by dt inside cosine.
    """
    phi = torch.exp(-dt / torch.clamp(tau, min=1e-12))  # [1,N]
    wd = freqs_rs * dt                                  # [F,1]
    denom = 1.0 + phi**2 - 2.0 * phi * torch.cos(wd)    # broadcasts to [F,N]
    return var / torch.clamp(denom, min=1e-30)
...
def _spectral_covariances(
    ...
    *,
    ...
    """
    Compute diagonal spectral PSDs C_a(ω), C_m(ω) for chosen kernels.
    All taus are in seconds. Frequencies are converted to radians/second by multiplying by dt.
    Returns C_a_fft, C_m_fft: [F, N]
    """
    F = freqs.shape[0]
    # Convert ω (rad/sample) -> rad/second by multiplying by dt
    freqs_rs = freqs.view(F, 1) * dt_seconds  # [F,1]

    # Shapes
    log_tau_a = log_tau_a.view(1, -1)  # [1,N] or [1,1]
    log_tau_m = log_tau_m.view(1, -1)
    var_a = torch.exp(log_var_a.view(1, -1))
    var_m = torch.exp(log_var_m.view(1, -1))
    tau_a = torch.exp(log_tau_a)
    tau_m = torch.exp(log_tau_m)

    # Activity PSD
    if kernel_a == KernelName.rbf:
        C_a = _psd_rbf(freqs_rs, tau_a, var_a)
    elif kernel_a == KernelName.ou_ct:
        C_a = _psd_ou_ct(freqs_rs, tau_a, var_a)
    elif kernel_a == KernelName.ar1_dt:
        dt_t = torch.tensor(dt_seconds, device=freqs.device, dtype=freqs.dtype).view(1,1)
        C_a = _psd_ar1_dt(freqs_rs, tau_a, var_a, dt_t)
    else:
        raise ValueError(f"Unsupported kernel_a: {kernel_a}")

    # Motion PSD
    if kernel_m == KernelName.rbf:
        C_m = _psd_rbf(freqs_rs, tau_m, var_m)
    elif kernel_m == KernelName.ou_ct:
        C_m = _psd_ou_ct(freqs_rs, tau_m, var_m)
    elif kernel_m == KernelName.ar1_dt:
        dt_t = torch.tensor(dt_seconds, device=freqs.device, dtype=freqs.dtype).view(1,1)
        C_m = _psd_ar1_dt(freqs_rs, tau_m, var_m, dt_t)
    else:
        raise ValueError(f"Unsupported kernel_m: {kernel_m}")
...
def _evidence_and_posterior(
    ...
    *,
    ...
    Inputs:
      ...
      log_tau_a, log_tau_m: [N] or [1] log timescales (seconds).
    Returns dict with:
      'neg_evidence': scalar,
...
def fit(
    ...
    """
    GPU-batched TMAC fit with timescale pooling.
    - Works in fold-change units (divide by per-ROI mean, subtract 1).
    - Taus are interpreted in *seconds*; supply options.dt as sampling interval in seconds.
    ...
    if options.pooling == PoolingMode.fixed:
        # tau_a / tau_m provided; accept scalar or per-ROI list (seconds)
        if options.tau_a is None or options.tau_m is None:
            raise ValueError("When pooling='fixed', tau_a and tau_m must be provided (seconds).")
    ...
    def _closure():
        ...
        if options.pooling == PoolingMode.hierarchical:
            mu_a = hier_mu_a
            mu_m = hier_mu_m
            sigma_a = hier_sigma_a
            sigma_m = hier_sigma_m
        else:
        ...
        out = _evidence_and_posterior(
            ...
            kernel_a=options.kernel_a,
            kernel_m=options.kernel_m,
        ...
        penalty = torch.tensor(0.0, device=red.device, dtype=red.dtype)
        if options.pooling == PoolingMode.hierarchical:
            penalty = (
                penalty
    ...
    result: Dict[str, Any] = {
        ...
        "taus": {},
        "kernels": {
            "a": options.kernel_a.value,
            "m": options.kernel_m.value,
        },
        "dt_seconds": float(options.dt),
    }
